...
async function fetchSelf(path: string): Promise<userProps> {
  ...
  async function fetchOther(path: string): Promise<userProps> {
    const token: Token = Cookies.getJSON('token')
    let config = {
      headers: {
        'Authorization': 'Bearer ' + token
...
export default function User() {
    ...
    if (username === token.iss) {
        let { data, error } = useSWR(`users/user`, fetchSelf);
        user = data;
    ...
    //quando é outro user 
      ...
      let { data, error } = useSWR(`users/get/${username}`, fetchOther);
      user = data;
      if (error) { return <SessionOf /> }
      if (!data) return <Loading />
      myLoader = () => {
